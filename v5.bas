...
D=(XM+YM)<7:T=XM<YM:K=PEEK(1024+X1+Y1*40)

IF D AND ((K=105) OR (K=233))     THEN X=X-Q:Y=Y-Q
IF NOT D AND ((K=105) OR (K=233)) THEN X=X+Q:Y=Y+Q
IF (T) AND ((K=95) OR (K=223))      THEN X=X-Q:Y=Y+Q
IF NOT (T) AND ((K=95) OR (K=223))  THEN X=X+Q:Y=Y-Q


REM IF D<=7 AND K=105 THEN X=X-Q:Y=Y-Q
REM IF D>7 AND K=105  THEN X=X+Q:Y=Y+Q
REM IF T<=7 AND K=233 THEN X=X+Q:Y=Y+Q
REM IF T>7 AND K=233  THEN X=X-Q:Y=Y-Q

X=X-C:Y=Y-U

REM POKE781,0:SYS59903
PRINT"{HOME}K:";K;" D:";D;" T:";XM>YM
